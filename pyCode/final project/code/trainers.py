...
import time
from utils import NDCG_binary_at_k_batch, Recall_at_k_batch, Precision_at_k_batch
...
def evaluate(model, criterion, data_tr, data_te, batch_size, N, device, total_anneal_steps, anneal_cap, is_VAE=False):
    ...
    with torch.no_grad():
        for start_idx in range(0, e_N, batch_size):
            ...
            p10 = Precision_at_k_batch(recon_batch, heldout_data, 10)
    ...
    p10_list = np.concatenate(p10_list)
    
    return total_loss, np.mean(p10_list), #np.mean(n100_list), np.mean(r20_list), np.mean(r50_list), 
