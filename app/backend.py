...
async def make_inference_track(request: Request, test: List):
    ...
    # model = EASE()
    model = load_model_vae_pt()
    input_ids1 = make_testfile(input_ids)
    result_ids = get_model_rec(model=model, input_ids=input_ids1, top_k=10)
    
    #lgbm
    model = load_model_lgbm_pt()
    test_file = make_testfile_lgbm()
    #train은 db에 있는 배치 item
    train = []
